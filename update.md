...
- 修复“因修复无法正常加载 Twikoo 引发的回归问题”，该问题会导致 Twikoo 无法自动刷新（该问题使用 GLM 5.3 Flash 修复）
- `Detail` 布局
    - `image` / `cover` 支持**相对路径**引用图片（`./xxx.webp`、`../assets/xxx.png`、裸文件名均可），以页面所在目录为基准解析，与 Markdown 正文相对图片的语义一致；构建期经 `import.meta.glob` 收集项目内图片资源，输出为带 hash 指纹的产物 URL，自动参与构建优化
    - `/` 绝对路径（public 资源）输出时自动补全 `base` 前缀，修复 `base` 非根路径部署下图片 404 的问题；完整 URL（`https://` 等）原样保留，不做处理
